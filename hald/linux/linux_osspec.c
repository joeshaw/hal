...
*/
	...
	/** @todo This is a hack; is there such a thing as an ide_host? */
			...
			if (cls != NULL) {
				if (cls->devices != NULL) {
					dlist_for_each_data (cls->devices,
							     class_device,
							     struct
							     sysfs_class_device)
					{
						printf
						    ("device->bus_id = %s, cls->name = %s\n",
						     device->bus_id,
						     class_device->name);
						if (strcmp
						    (device->bus_id,
						     class_device->name) ==
						    0)
							visit_class_device_i2c_adapter
							    (class_device->
							     path,
							     class_device);
					}
...
handle_hotplug (DBusConnection * connection, DBusMessage * message)
{
	...
	if (sysfs_devpath[0] != '\0' &&
		...
		if (is_add) {
			HAL_INFO (("Adding classdevice @ sysfspath %s",
				   sysfs_devpath));
			...
			d = ds_device_find_by_key_value_string
			    ("linux.sysfs_path", sysfs_devpath, TRUE);
			if (d == NULL) {
				HAL_WARNING (("Couldn't remove device @ %s on "
					      "hotplug remove",sysfs_devpath));
			} else {
				if (strcmp (subsystem, "block") == 0)
					linux_class_block_removed (d);

				HAL_INFO (("Removing classdevice @ sysfspath "
					   "%s, udi %s",sysfs_devpath,d->udi));
				ds_device_destroy (d);
			}
		}
	} else if (strcmp (subsystem, "input") == 0) {
		if (is_add) {
			/** @todo FIXME when kernel 2.6 got input devices in
			 *        sysfs this terrible hack can be removed
			 */
			linux_class_input_handle_hotplug_add (input_name,
							      input_phys,
							      input_key,
							      input_ev,
							      input_rel,
			...
			/* This block is left intentionally blank, since input
			 * class devices simply attach to other (physical)
			 * devices and when they disappear, the input part 
			 * also goes away
			 */
		}
	}
			  ...
			  DBusMessage * message)
{
	...
	if (dbus_message_get_args (message, NULL,
		...
		/* Find block device; this happens asynchronously as our it 
		 * might be added later..
		 */
		ds_device_async_find_by_key_value_string (
			"linux.sysfs_path_device", sysfs_dev_path, FALSE,
				       ...
				       void *data1, void *data2)
{
	...
	if (d != NULL) {
		HAL_INFO (("Setting block.device=%s for udi=%s", filename,
			   d->udi));
...
*/
	...
	if (dbus_message_is_method_call (message,
		...
		(message, "org.kernel.udev.NodeMonitor", "NodeDeleted")) {
