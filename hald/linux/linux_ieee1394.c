 ...
 */
static char *
ieee1394_compute_udi (HalDevice * d, int append_num)
{
	const char *serial;
	const char *format;
	static char buf[256];
	...
	if (append_num == -1) {
		snprintf (buf, 256,
			  "/org/freedesktop/Hal/devices/ieee1394_%s",
			  ds_property_get_string (d,
						  "linux.sysfs_bus_id"));
	} else {
		snprintf (buf, 256,
			  "/org/freedesktop/Hal/devices/ieee1394_%s-%d",
 ...
 */
static void
visit_device_ieee1394_got_parent (HalDevice * parent,
 ...
 */
void
visit_device_ieee1394 (const char *path, struct sysfs_device *device)
{
	...
	ds_device_async_find_by_key_value_string
	    ("linux.sysfs_path_device", parent_sysfs_path, TRUE,
	     visit_device_ieee1394_got_parent, (void *) d, NULL,
